...
function icon(mime){
    ...
    if(isFolderMime(mime)) return 'bi-folder-fill text-warning';
...
function isFolderMime(mime){ return String(mime||'').toLowerCase().includes('folder'); }

function renderBreadcrumb() {
    const nav = document.getElementById('breadcrumbNav');
    if(!nav) return;
    nav.innerHTML = F.folderPath.map((folder, index) => {
        const label = esc(folder.name);
        if(index === F.folderPath.length - 1) return `<li class="breadcrumb-item active">${label}</li>`;
        return `<li class="breadcrumb-item"><a href="#" data-folder-index="${index}">${label}</a></li>`;
    }).join('');
    nav.querySelectorAll('[data-folder-index]').forEach(link => {
        link.addEventListener('click', e => {
            e.preventDefault();
            F.folderPath = F.folderPath.slice(0, Number(link.dataset.folderIndex) + 1);
            F.currentFolderId = F.folderPath[F.folderPath.length - 1].id;
            loadFiles();
        });
    });
}
...
function openFolder(id, name) {
    F.currentFolderId = id;
    F.folderPath.push({ id, name });
    loadFiles();
}
...
async function getApiList(url) {
    ...
    let nextUrl = url;
    while(nextUrl) {
        const r = await axios.get(nextUrl, { headers: authHeaders() });
        const data = r.data || [];
...
async function loadFiles() {
    ...
    try {
        const url = F.currentFolderId
            ...
            : '/api/v1/files/';
        let files = await getApiList(url);
        if(!F.currentFolderId) files = files.filter(f => !f.parent);
        if (F.searchQuery) {
...
function renderTable(files) {
    ...
    const folder = isFolderMime(f.mime_type);
    return `
    <tr class="file-row" data-id="${f.id}">
        <td>
            <div class="d-flex align-items-center gap-2">
                ...
                <div>
                    <button class="btn btn-link p-0 fw-semibold text-start text-truncate ${folder?'folder-open-btn':'preview-btn'}" style="max-width:220px;text-decoration:none" data-id="${f.id}" data-mime="${esc(f.mime_type||'')}" data-name="${esc(f.name)}" title="${esc(f.name)}">${esc(f.name)}</button>
        ...
        <td class="text-end">
            <div class="btn-group btn-group-sm">
                <button class="btn btn-outline-secondary ${folder?'folder-open-btn':'preview-btn'}" data-id="${f.id}" data-mime="${esc(f.mime_type||'')}" data-name="${esc(f.name)}" title="${folder?'Open':'Preview'}"><i class="bi ${folder?'bi-folder2-open':'bi-eye'}"></i></button>
                <button class="btn btn-outline-primary download-btn" data-id="${f.id}" data-name="${esc(f.name)}" title="Download" ${folder?'disabled':''}><i class="bi bi-download"></i></button>
...
function renderGrid(files) {
    ...
    grid.innerHTML = `<div class="row g-3">${files.map(f=>`
        <div class="col-6 col-md-4 col-lg-3">
            <div class="card h-100 shadow-sm file-grid-card" style="cursor:pointer" data-id="${f.id}" data-name="${esc(f.name)}" data-folder="${isFolderMime(f.mime_type)}">
                ...
                <div class="card-footer bg-transparent d-flex justify-content-around p-1">
                    <button class="btn btn-sm btn-link ${isFolderMime(f.mime_type)?'folder-open-btn':'preview-btn'}" data-id="${f.id}" data-mime="${esc(f.mime_type||'')}" data-name="${esc(f.name)}"><i class="bi ${isFolderMime(f.mime_type)?'bi-folder2-open':'bi-eye'}"></i></button>
...
function bindTableActions() {
    document.querySelectorAll('.folder-open-btn').forEach(btn => {
        btn.onclick = e => { e.stopPropagation(); openFolder(btn.dataset.id, btn.dataset.name); };
    });
    document.querySelectorAll('.file-grid-card').forEach(card => {
        card.ondblclick = () => {
            if(card.dataset.folder === 'true') openFolder(card.dataset.id, card.dataset.name);
        };
    });
...
function populateFolderPicker() {
    ...
    if(!select) return;
    const folders = F.allFiles.filter(f => (f.mime_type||'').includes('folder'));
    const folderMap = Object.fromEntries(folders.map(f => [String(f.id), f]));
    function getPath(folder) {
        const parts = [];
        let cur = folder;
        while(cur) { parts.unshift(cur.name); cur = cur.parent ? folderMap[String(cur.parent)] : null; }
        return parts.join(' / ');
    }
    const sorted = folders.map(f => ({...f, _path: getPath(f)})).sort((a,b) => a._path.localeCompare(b._path));
    const current = F.currentFolderId
        ? F.folderPath[F.folderPath.length - 1]
        : null;
    const hasCurrent = current && sorted.some(f => String(f.id) === String(current.id));
    select.innerHTML = `<option value="">— My Drive (root) —</option>` +
        (hasCurrent ? '' : current ? `<option value="${current.id}">${esc(F.folderPath.map(f => f.name).join(' / '))}</option>` : '') +
        sorted.map(f => `<option value="${f.id}">${esc(f._path)}</option>`).join('');
    if(F.currentFolderId) select.value = F.currentFolderId;
}

// --- UPLOAD MODAL ---
function setupUpload() {
    const modalEl   = document.getElementById('uploadModal');
    const fileInput = document.getElementById('uploadModalFileInput');
    const dropzone  = document.getElementById('uploadModalDropzone');
    const fileList  = document.getElementById('uploadFileList');
    const confirmBtn = document.getElementById('uploadModalConfirm');
    const folderSelect = document.getElementById('uploadFolderSelect');
    const bar = document.getElementById('uploadProgress');

    // Legacy hidden input still works
    const legacyInput = document.getElementById('fileUploadInput');
    if(legacyInput && !legacyInput.dataset.bound) {
        legacyInput.dataset.bound = 'true';
    ...
    if(!modalEl) {
        // Fallback: wire upload button directly
        const btn = document.getElementById('uploadBtn');
        if(btn) btn.addEventListener('click', () => legacyInput?.click());
    ...
    function refreshFileList() {
        ...
        fileList.innerHTML = pendingFiles.map((f, i) =>
            `<li class="list-group-item d-flex justify-content-between align-items-center py-1 px-0">
                <span class="small text-truncate me-2"><i class="bi bi-file-earmark me-1"></i>${esc(f.name)}</span>
                <button class="btn btn-sm btn-link text-danger p-0" data-remove="${i}"><i class="bi bi-x-lg"></i></button>
            </li>`
        ).join('');
        fileList.querySelectorAll('[data-remove]').forEach(btn => {
            btn.addEventListener('click', () => { pendingFiles.splice(parseInt(btn.dataset.remove), 1); refreshFileList(); });
        });
    }

    if(fileInput) fileInput.addEventListener('change', e => {
        pendingFiles.push(...Array.from(e.target.files||[])); refreshFileList(); e.target.value='';
    });
    if(dropzone) {
        dropzone.addEventListener('click', () => fileInput?.click());
        dropzone.addEventListener('dragover', e => { e.preventDefault(); dropzone.classList.add('bg-light'); });
        dropzone.addEventListener('dragleave', () => dropzone.classList.remove('bg-light'));
        dropzone.addEventListener('drop', e => { e.preventDefault(); dropzone.classList.remove('bg-light'); pendingFiles.push(...Array.from(e.dataTransfer.files||[])); refreshFileList(); });
    }

    modalEl.addEventListener('show.bs.modal', () => { pendingFiles = []; refreshFileList(); populateFolderPicker(); });
    ...
    if(confirmBtn) confirmBtn.addEventListener('click', async () => {
        ...
        try {
            if(bar){ bar.style.display='block'; bar.style.width='0%'; }
            await axios.post('/api/v1/files/upload_multiple/', form, {
                headers: { Authorization: `Token ${tok()}` },
                onUploadProgress(e){ if(bar) bar.style.width=`${Math.round(e.loaded/e.total*100)}%`; }
            });
            ...
            confirmBtn.innerHTML = '<i class="bi bi-cloud-arrow-up me-1"></i>Upload';
    ...
    if(uploadBtn) uploadBtn.addEventListener('click', () => new bootstrap.Modal(modalEl).show());

    // Global drag & drop still works
    document.addEventListener('dragover', e => e.preventDefault());
...
function initFilesPage() {
    ...
    loadFiles().then(() => populateFolderPicker());
    setupSearch();
    setupViewToggle();
    setupUpload();
    setupFolderBtn();
